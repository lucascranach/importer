...
/**
 * Extends drawings keywords with parent terms from thesaurus hierarchy
 * If a drawing has keyword "01050501" (Clair-obscure), it will also get "010505" (Holzschnitt)
 */
class KeywordParentTermExtender extends Hybrid
{
    ...
    private function extendCollectionWithParentTerms(DrawingLanguageCollection $collection): void
    {
        /** @var \CranachDigitalArchive\Importer\Modules\Drawings\Interfaces\IDrawing $drawing */
        foreach ($collection as $drawing) {
            $existingKeywords = $drawing->getKeywords();
            $newKeywords = [];
            ...
            // Add all new parent keywords to the drawing
