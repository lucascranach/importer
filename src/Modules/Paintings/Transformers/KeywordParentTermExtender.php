...
/**
 * Extends paintings keywords with parent terms from thesaurus hierarchy
 * If a painting has keyword "01050501" (Clair-obscure), it will also get "010505" (Holzschnitt)
 */
class KeywordParentTermExtender extends Hybrid
{
    ...
    private function extendCollectionWithParentTerms(PaintingLanguageCollection $collection): void
    {
        /** @var \CranachDigitalArchive\Importer\Modules\Paintings\Interfaces\IPainting $painting */
        foreach ($collection as $painting) {
            $existingKeywords = $painting->getKeywords();
            $newKeywords = [];
            ...
            // Add all new parent keywords to the painting
